...
export default function Login(){
    ...
    function handleSubmit(e) {
        ...
        AuthService.login(usern, pass).then(
            ()=> {
                navigate("/searchreport")
            }
        )
    }
    ...
    <div className="flex justify-center items-center  h-[100vh] ">
        ...
        <div className="font-semibold text-xl mb-6 ">Login Form</div>  
        <form id="login" onSubmit={handleSubmit} className="flex flex-col items-center mb-8">
            ...
            <button className="bg-slate-600 rounded-md w-[40%] h-[35px] text-sm text-white drop-shadow-xl place-self-start " type="submit">Sign In</button>
        </form>
        <div className="text-xs">Belum punya akun? <Link className="text-cyan-600" to="/register">Sign Up here</Link> </div>
    </div>
    </div>
